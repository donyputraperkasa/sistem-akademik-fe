"use client";

import { useState } from "react";

type Student = {
  id: number;
  fullName: string;
  nickname: string;
  birthDate: string;
  className: string;
};

type StudentFormProps = {
  students: Student[];
  setStudents: React.Dispatch<React.SetStateAction<Student[]>>;
};

export default function StudentForm({ students, setStudents }: StudentFormProps) {
  const [form, setForm] = useState({
    fullName: "",
    nickname: "",
    birthDate: "",
    className: "",
  });

  const handleAdd = () => {
    if (!form.fullName || !form.nickname || !form.birthDate || !form.className) return;
    const newStudent = { id: students.length + 1, ...form };
    setStudents([...students, newStudent]);
    setForm({ fullName: "", nickname: "", birthDate: "", className: "" });
  };

  return (
    <div className="mb-6 bg-blue-50 p-4 rounded-lg shadow-sm">
      <div className="bg-white p-4 rounded-md shadow-sm">
        <h2 className="font-medium mb-2">Tambah Siswa Baru</h2>
        <div className="grid grid-cols-4 gap-3">
          <input
            type="text"
            placeholder="Nama Lengkap"
            className="border rounded-md p-2"
            value={form.fullName}
            onChange={(e) => setForm({ ...form, fullName: e.target.value })}
          />
          <input
            type="text"
            placeholder="Nama Panggilan"
            className="border rounded-md p-2"
            value={form.nickname}
            onChange={(e) => setForm({ ...form, nickname: e.target.value })}
          />
          <input
            type="date"
            className="border rounded-md p-2"
            value={form.birthDate}
            onChange={(e) => setForm({ ...form, birthDate: e.target.value })}
          />
          <input
            type="text"
            placeholder="Kelas"
            className="border rounded-md p-2"
            value={form.className}
            onChange={(e) => setForm({ ...form, className: e.target.value })}
          />
        </div>
      </div>
      <button
        onClick={handleAdd}
        className="mt-3 bg-blue-600 text-white py-2 px-4 rounded-md hover:bg-blue-700"
      >
        Tambah
      </button>
    </div>
  );
}
